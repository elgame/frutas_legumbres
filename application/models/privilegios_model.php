...
class privilegios_model extends CI_Model{
	...
	public function generaMenuPrivilegio($id_submenu=0, $firs=true){
		...
		$res = $this->db
			...
			->from('privilegios AS p')
				->join('usuarios_privilegios AS ep','p.id = ep.privilegio_id','inner')
			->where("ep.usuario_id = '".$this->session->userdata('id')."' AND p.id_padre = '".$id_submenu."' AND mostrar_menu = 1")
			->order_by('p.nombre', 'asc')
		->get();
		foreach($res->result() as $data){
			$res1 = $this->db
				...
				->from('privilegios AS p')
					->join('usuarios_privilegios AS ep','p.id = ep.privilegio_id','inner')
				->where("ep.usuario_id = '".$this->session->userdata('id')."' AND p.id_padre = '".$data->id."' AND mostrar_menu = 1")
			->get();
			$data1 = $res1->row();
